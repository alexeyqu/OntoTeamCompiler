...
#include "IRTree/Wrappers.h"
#include <assert.h>
#include <vector>

struct CFragment {
	CFragment( CFrame* _frame, IStm* _rootStm, std::string& _name ) :
...
{
public:
	CTranslateVisitor( CSymbolsTable* _table ) :
		table( _table ), curFrame( nullptr ), curClass( nullptr ), 
		curMethod( nullptr ), expList( nullptr ) {
		assert( table );
	}
...
private:
	CSymbolsTable* table;
	CFrame* curFrame;
	CClassSymbol* curClass;
	CMethodSymbol* curMethod;
